...
Evaluate the explanation according to the following dimensions:

1. feature_faithfulness
To what extent does the explanation focus on the important features identified by the explainer?

2. prediction_consistency
To what extent does the explanation logically support the enhancer prediction?

3. unsupported_claims
To what extent does the explanation avoid unsupported, contradictory, or clinically implausible claims?

4. clarity
How clear and understandable is the explanation for a clinician?

Assign an integer score from 1 (very poor) to 5 (excellent) for each evaluation dimension.
...
How relevant are the retrieved documents for supporting the enhancer prediction and generated explanation?

2. grounding
To what extent is the generated explanation grounded in the retrieved documents?

3. unsupported_claims
To what extent does the generated explanation avoid unsupported claims with respect to the retrieved documents?
...
""".strip()
    ...
    def process_answer(
        ...
        result.update({
            ...
            f"{self.judge_name}_answer_dict": np.nan,
    ...
    def _parse_json_answer(answer: str) -> Dict[str, Any]:
        ...
        json_text = match.group(0).replace(r"\_", "_")
