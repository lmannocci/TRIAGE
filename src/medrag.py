...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False):
        ...
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            #     "cpu"
            # )
            # print(f"Device transformers.pipeline: {device}")
            # self.model = transformers.pipeline(
            #     "text-generation",
            #     model=self.llm_name,
            #     torch_dtype=torch.bfloat16 if device == "cuda" else torch.float32,  # MPS doesn't support bfloat16 well
            #     device=0 if device == "cuda" else -1,  # Ensure correct device selection
    ...
    def medrag_answer(self, question, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, **kwargs):
        ...
        if self.rag:
            ...
            print("DocExtracter completed")
            print("tokenizer decode start")
            ...
            print("tokenizer decode completed")
        else:
            retrieved_snippets = []
            scores = []
            contexts = []
        ...
        if not self.rag:
            ...
            for ind, context in enumerate(contexts):
                print(f"context: {ind}/{len(contexts)}")
